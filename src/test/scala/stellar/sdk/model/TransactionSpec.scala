...
class TransactionSpec extends Specification with ArbitraryInput with DomainMatchers {
  ...
  "a transaction" should {
    ...
    "serde via xdr bytes" >> prop { transaction: Transaction =>
      Transaction.decodeXdr(transaction.xdr) mustEqual transaction
    }

    "allow adding of operations one at a time" >> prop { (source: Account, ops: List[Operation]) =>
    ...
    "calc the minFee as 100 stroops * the quantity of operations" >> prop { (source: Account, ops: List[Operation]) =>
    ...
    "disallow signing if the maxFee is insufficient" >> prop { (source: Account, ops: List[Operation], signer: KeyPair) =>
    ...
    "express signed transaction envelope as base64" >> {
      ...
      val txn = Transaction(
        source = account,
        operations = List(CreateAccountOperation(dest.toAccountId, NativeAmount(20000000000L))),
        timeBounds = Unbounded,
        maxFee = NativeAmount(100)
      )(TestNetwork).sign(source)

      txn.transaction.source must beEquivalentTo(account)
      txn.encodeXDR mustEqual "AAAAAgAAAABexSIg06FtXzmFBQQtHZsrnyWxUzmthkBEhs/ktoeVYgAAAGQAClWjAAAAAQAAAAAAAAAAAAAAA" +
        "QAAAAAAAAAAAAAAAO3gUmG83C+VCqO6FztuMtXJF/l7grZA7MjRzqdZ9W8QAAAABKgXyAAAAAAAAAAAAbaHlWIAAABAy5IvTou9NDetC6PI" +
        "FJhBR2yr2BuEEql4iyLfU9K7tjuQaYVZf40fbWLRwA/lHg2IYFzYMFMakxLtVrpLxMmHAw=="
    }
  }
  ...
  "signing a transaction with a keypair" should {
    "add a signature to that transaction" >> prop { (source: Account, op: Operation, signers: Seq[KeyPair]) =>
      val signatures = model.Transaction(source, List(op), NoMemo, timeBounds = Unbounded, maxFee = NativeAmount(100))
    ...
    "verify for the signed keypairs" >> prop { (source: Account, op: Operation, signers: Seq[KeyPair]) =>
      val signedTransaction =
        model.Transaction(source, List(op), NoMemo, timeBounds = Unbounded, maxFee = NativeAmount(100))
  ...
  "signing a transaction with a pre-image" should {
    "add a signature to that transaction" >> prop { (source: Account, op: Operation, preImages: Seq[Array[Byte]]) =>
      val transaction = model.Transaction(source, List(op), NoMemo, timeBounds = Unbounded, maxFee = NativeAmount(100))
...
//      txn mustEqual Transaction.decodeXdr(txn.xdr)
