...
import org.stellar.xdr.{ClaimPredicate => XClaimPredicate, ClaimPredicateType, Int64}
...
object ClaimPredicate {
  ...
  def decode(xdr: XClaimPredicate): ClaimPredicate =
    xdr.getDiscriminant match {
      ...
      case ClaimPredicateType.CLAIM_PREDICATE_AND =>
        And(decode(xdr.getAndPredicates().head), decode(xdr.getAndPredicates()(1)))
      case ClaimPredicateType.CLAIM_PREDICATE_NOT =>
        Not(decode(xdr.getNotPredicate))
      case ClaimPredicateType.CLAIM_PREDICATE_OR =>
        Or(decode(xdr.getOrPredicates().head), decode(xdr.getOrPredicates()(1)))
      ...
      case ClaimPredicateType.CLAIM_PREDICATE_BEFORE_RELATIVE_TIME =>
        SinceClaimCreation(xdr.getAbsBefore.getInt64)
    }
